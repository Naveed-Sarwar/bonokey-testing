...
type Props = {
    ...
    searchFunction: (processedMonthlySalary: number, processedLoanAmount: number, processedJobSector: string, processedDuration: number, processedUseMaxLoan: boolean) => Promise<void> | null;
...
const InputDiv = styled.div<{ selected: boolean }>`
    height: 100%;
    background-color: ${(props) => (props.selected ? brandColors.captionBlack + "32" : brandColors.transparent)};
    border-radius: 5px;
    padding: 8px 24px;
    display: grid;
...
function SearchBar(props: Props) {
    const [monthlySalary, setMonthlySalary] = React.useState(props.monthlySalary ? props.monthlySalary.toString() : "8500");
    const [monthlySalaryFocused, setMonthlySalaryFocused] = React.useState(false);
    const [isEditingMonthlySalary, setIsEditingMonthlySalary] = React.useState(false);
    ...
    const [loanAmount, setLoanAmount] = React.useState(props.loanAmount ? props.loanAmount.toString() : "100000");
    ...
    const loanAmountHandleInputClick = () => {
        if (loanAmountInputRef.current) {
            loanAmountInputRef.current.setSelectionRange(loanAmountInputRef.current.value.lastIndexOf("SAR") - 1, loanAmountInputRef.current.value.lastIndexOf("SAR") - 1);
        }
    };

    const [jobSector, setJobSector] = React.useState(props.jobSector ? props.jobSector : "Government");
    const [jobSectorFocused, setJobSectorFocused] = React.useState(false);

    const [duration, setDuration] = React.useState(props.duration ? props.duration : 5);
    const [durationFocused, setDurationFocused] = React.useState(false);

    const enableSearchButton = Number(monthlySalary) > 0 && Number(loanAmount) > 0;

    return (
        <ContentContainerDiv>
            <SearchBarDiv data-aos={props.isOnHomePage ? "fade-up" : ""} data-aos-delay={props.isOnHomePage ? "300" : ""}>
                <InputGridDiv>
                        ...
                        {isEditingMonthlySalary ? (
                            <div style={{ display: "flex", width: "100%" }}>
                                <TextInput
                                    ...
                                    onChange={(event) => {
                                        ...
                                        const modValue = value.replace("SAR", "").replaceAll(" ", "");
                                        if (modValue === "" || regex.test(modValue)) {
                                            setMonthlySalary(Number(modValue) > 90000 ? Number(90000).toString() : modValue);
                                        }
                                    }}
                                />
                            ...
                            <div style={{ display: "flex", width: "100%" }}>
                                <TextInput
                                    ...
                                    value={`${monthlySalary ? Number(monthlySalary).toLocaleString(undefined) : ""}`}
                                />
                                <div style={{ marginLeft: "-32px" }}>
                                    <p style={{ fontSize: "1rem", fontWeight: "400" }}>SAR</p>
                                </div>
                        ...
                        {isEditingLoanAmount ? (
                            <div style={{ display: "flex" }}>
                                <TextInput
                                    ...
                                    onChange={(event) => {
                                        ...
                                        const modValue = value.replace("SAR", "").replaceAll(" ", "");
                                        if (modValue === "" || regex.test(modValue)) {
                                            setLoanAmount(Number(modValue) > 1000000 ? Number(1000000).toString() : modValue);
                                        }
                                    }}
                                ...
                                />
                                <div
                                    style={{ marginLeft: "-89px", paddingBottom: "6px", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}
                            ...
                            <div style={{ display: "flex", width: "100%" }}>
                                <TextInput
                                    ...
                                    value={`${loanAmount ? Number(loanAmount).toLocaleString(undefined) : ""}`}
                                    style={{ paddingRight: "55px", width: "100%" }}
                                    disabled={useMaxLoan}
                                />
                                <div
                    ...
                    <InputDiv selected={false}>
                        {props.isOnHomePage && !props.searchFunction ? (
                            ...
                            >
                                <SearchButton disabled={!enableSearchButton}>Search</SearchButton>
                            </Link>
                        ) : (
                            <SearchButton
                                disabled={!enableSearchButton}
                                onClick={() => {
                                    props.searchFunction(Number(monthlySalary), Number(loanAmount), jobSector, Number(duration), useMaxLoan);
                                }}
                            >
                                Search
                            </SearchButton>
